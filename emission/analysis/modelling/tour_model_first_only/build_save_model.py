...
def create_user_input_map(trip_list, bins):
    ...
    for b, curr_bin in enumerate(bins):
        ...
        user_label_df = pd.DataFrame(
            [trip['data']['user_input'] for trip in bin_trips])
        # skip the label mapping from sp to en since we want to preserve original labels for the performance evaluation
        # user_label_df = lp.map_labels(user_label_df).dropna()
        user_label_df = user_label_df.dropna()
        ...
        unique_labels = user_label_df.groupby(
            user_label_df.columns.tolist()).size().reset_index(
                name='uniqcount')
        ...
        for i in range(len(unique_labels)):
            ...
            labels_only = {
                column: unique_labels.iloc[i][column]
